...
/**
 * This class implements a distance measure for geopoints based on latitude,
 * longitude and heading.
 * 
 * The difference is determined in kilometers, based on the great circle
 * distance. Points with a heading difference that is too large will be assigned
...
public class POIDistanceMeasure implements DistanceMeasure {
	...
	public static final int METER_FACTOR = 1000;

	/**
	 * Calculates distance as meter between two positions
	 * 
	 * @param arg0
	 *            first position
	 * @param arg1
	 ...
	 * @return
	 */
	public double computeDistance(double[] arg0, double[] arg1) {
		return haversine(arg0[0], arg0[1], arg1[0], arg1[1]) * METER_FACTOR;
	}

	@Override
	public double compute(double[] arg0, double[] arg1) throws DimensionMismatchException {
		// compute heading diff first and see if these points shall be
		// clusterable.
		double headingA = arg0[2];
		double headingB = arg1[2];
		double headingDiff = headingDiff(headingA, headingB);

		// points shall only be clusterable if their heading diff is not too
		// large
	...
	/**
	 * Compute the great circle distance between two points given by latitude
	...
	/**
	 * Computes the difference of two given headings in degree between 0 and
	 * 360. It does account for the headings making circulating around the
	 * 360deg as follows.
	 * 
	 * Examples from this method the differences in degrees are 350deg to 10deg
	 * => 20 170deg to 190deg => 20 10deg to 5deg => 5
	 *
	 * @param headingA
	 *            The heading as double in [0.0, 360.0]
	 * @param headingB
	 *            The heading as double in [0.0, 360.0]
